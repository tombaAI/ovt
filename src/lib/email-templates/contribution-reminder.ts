/**
 * HTML šablona připomínkového emailu k zaplacení příspěvků OVT.
 * Odesílá se členům, kteří dosud nezaplatili, před termínem splatnosti.
 */

import type { ContribEmailData } from "./contribution-prescription";
import { vocative } from "./vocative";

export type ContribReminderEmailData = ContribEmailData & {
    latePenalty: number;   // poplatek z prodlení po splatnosti (0 = nezobrazovat)
};
...
function buildPayliboUrl(data: ContribReminderEmailData): string | null {
...
export function buildContributionReminderEmail(data: ContribReminderEmailData): { subject: string; html: string } {
...
<table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;max-width:560px;width:100%;">
  ...
  <tr>
    <td style="background:#327600;padding:24px 32px;">
      ...
      <p style="margin:4px 0 0;color:#a3d977;font-size:14px;">Příspěvky ${data.year} — připomínka</p>
    </td>
  </tr>
  ...
  <tr>
    <td style="padding:28px 32px 8px;">
      ...
      <p style="margin:0 0 8px;font-size:14px;color:#6b7280;line-height:1.6;">
        posíláme ti přátelskou připomínku, že členské příspěvky OVT Bohemians na rok
