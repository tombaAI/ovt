...
import {
    ...
    type MemberFormState, type AuditEntry, type MemberYearRecord, type MemberEventReg,
                    ...
                    {rows.map(r => {
                        ...
                        return (
                            <>
                                ...
                                >
                                    <td className="py-1.5 pr-3 font-semibold text-gray-800 flex items-center gap-1">
                                        {hasPayments && (
                                ...
                                {isExpanded && r.payments.map(p => (
                                    <tr key={p.id} className="border-b last:border-0 bg-muted/20">
                                        <td className="py-1 pl-5 pr-3 text-xs text-gray-400">
                                            {p.paidAt ? fmtDateShort(p.paidAt) : "—"}
                                            <span className="ml-1.5 text-[10px] px-1 rounded bg-gray-100 text-gray-500">{fmtSource(p.sourceType)}</span>
                                            {p.isSuggested && <span className="ml-1 text-[10px] text-amber-500">návrh</span>}
                                        </td>
                                        <td colSpan={2} className="py-1 pr-3 text-xs text-gray-500 italic">{p.note ?? ""}</td>
                                        <td className="py-1 text-right font-mono text-xs font-semibold text-gray-700">
...
function EventHistory({ memberId }: { memberId: number }) {
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                                ...
                                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5 text-xs text-gray-500">
                                    <span>{r.year}</span>
                                    {r.dateFrom && <span>{fmtDateShort(r.dateFrom)}{r.dateTo && r.dateTo !== r.dateFrom ? ` – ${fmtDateShort(r.dateTo)}` : ""}</span>}
                                    <span className="px-1 rounded bg-gray-100 text-gray-500">{EVENT_TYPE_LABELS[r.eventType] ?? r.eventType}</span>
