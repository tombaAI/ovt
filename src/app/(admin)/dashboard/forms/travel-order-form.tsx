...
import { WandSparkles, Download, BusFront, RotateCcw } from "lucide-react";
...
type TextField = Exclude<keyof CestovniPrikazData, "nakladyNaDopravu">;
...
export function TravelOrderForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/pdf/cestovni-prikaz", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          nazevAkce: normalizeText(form.nazevAkce),
          id: normalizeText(form.id),
          nakladyNaDopravu: form.nakladyNaDopravu,
      ...
      downloadBlob(blob, "cestovni-prikaz.pdf");
    } catch (error) {
  ...
  return (
    <Card className="overflow-hidden border-none py-0 ring-1 ring-indigo-100">
      <CardHeader className="border-b bg-gradient-to-r from-indigo-50 via-white to-indigo-50 py-5">
        <CardTitle className="flex items-center gap-2 text-lg">
          <BusFront className="size-5 text-indigo-700" />
          Cestovní příkaz
        </CardTitle>
      </CardHeader>
      ...
      <form onSubmit={submit}>
        <CardContent className="space-y-6 py-6">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <div className="rounded-xl border bg-slate-50 p-3">
              <div className="text-xs text-muted-foreground">Náklady na dopravu</div>
              <div className="mt-1 text-lg font-semibold">{formatCzk(form.nakladyNaDopravu ?? 0)}</div>
            </div>
            <div className="rounded-xl border bg-slate-50 p-3">
              <div className="text-xs text-muted-foreground">ID akce</div>
              <div className="mt-1 text-lg font-semibold">{form.id ?? "-"}</div>
            </div>
            <div className="rounded-xl border bg-indigo-50 p-3">
              <div className="text-xs text-muted-foreground">Doporučený VS</div>
              <div className="mt-1 text-lg font-semibold text-indigo-700">{suggestedVs || "-"}</div>
            </div>
          </div>

          <div className="rounded-xl border p-4">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold">Akce a náklad</h3>
              ...
              <div className="space-y-1.5 md:col-span-1">
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    setForm((prev) => ({ ...prev, nakladyNaDopravu: parsed }));
                  }}
                />
              </div>
            </div>
        ...
        <CardFooter className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs text-muted-foreground">
            Po stažení se dokument otevře s podpisovými poli.
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button type="submit" disabled={downloading || !canSubmit}>
              ...
              {downloading ? "Generuji PDF..." : "Stáhnout cestovní příkaz"}
            </Button>
          </div>
        </CardFooter>
      </form>
